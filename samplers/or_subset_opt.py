...
def get_solution_as_tensor(
    x: List[List[pywraplp.Variable]], num_nodes: int, device: torch.device = "cpu"
):
    xs = [
        [x[j][i].solution_value() for i in range(len(x[0]))] for j in range(num_nodes)
    ]
...
def get_or_assignment(scores: torch.Tensor, nnodes_list: List, k: int) -> torch.Tensor:
    ...
    x = [
        [solver.IntVar(0, 1, f"x[{j}][{i}]") for i in range(n_centroids)]
        for j in range(max_nnodes)
    ]
        ...
        if nnodes_list[ig] > last_nnodes:
            for cons_id in range(n_centroids):
                for node_id in range(last_nnodes, nnodes_list[ig]):
                    solver.constraint(cons_id).SetCoefficient(x[node_id][cons_id], 1.0)
        # remove some ones from the constraints
        elif nnodes_list[ig] < last_nnodes:
            for cons_id in range(n_centroids):
                for node_id in range(nnodes_list[ig], last_nnodes):
                    solver.constraint(cons_id).SetCoefficient(x[node_id][cons_id], 0.0)
